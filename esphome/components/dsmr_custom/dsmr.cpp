...
//                               this->crc_check_);  // Parse telegram according to data definition. Ignore unknown
//                               values.
//   if (res.err) {
...
bool DsmrCustom::available_within_timeout_() {
  ...
  if (this->parent_->get_rx_buffer_size() < this->max_telegram_len_) {
    while (!this->receive_timeout_reached_()) {
      // Reemplazo de delay() para ESP-IDF
...
void DsmrCustom::start_requesting_data_() {
  if (!this->requesting_data_) {
    ...
    this->requesting_data_ = true;
    this->last_request_time_ = this->millis_();
  }
}
...
void DsmrCustom::receive_encrypted_telegram_() {
  while (this->available_within_timeout_()) {
    ...
    if (this->crypt_telegram_len_ == 0 && this->crypt_bytes_read_ > 12) {
      this->crypt_telegram_len_ = 13 + (this->crypt_telegram_[11] << 8 | this->crypt_telegram_[12]);
      ESP_LOGV(TAG, "Encrypted telegram length: %d bytes", this->crypt_telegram_len_);
    }
    ...
    // --- Inicio del bloque de desencriptación con mbedTLS ---
    mbedtls_gcm_context gcm;
    mbedtls_gcm_init(&gcm);

    int ret = mbedtls_gcm_setkey(&gcm, MBEDTLS_CIPHER_ID_AES, this->decryption_key_.data(), 128);
    if (ret != 0) {
      ESP_LOGE(TAG, "mbedtls_gcm_setkey failed: -0x%x", -ret);
      mbedtls_gcm_free(&gcm);
      this->reset_telegram_();
      return;
    }

    // El IV (vector de inicialización) se compone del título del sistema y el contador de trama
    unsigned char iv[12];
    memcpy(iv, &this->crypt_telegram_[2], 8);       // Título del sistema (8 bytes)
    memcpy(iv + 8, &this->crypt_telegram_[14], 4);  // Contador de trama (4 bytes) - Ajustado según la nueva lógica

    size_t ciphertext_len = this->crypt_bytes_read_ - 18;  // El texto cifrado comienza en el byte 18
    unsigned char *ciphertext = &this->crypt_telegram_[18];
    unsigned char *decrypted_text = reinterpret_cast<unsigned char *>(this->telegram_);

    // La etiqueta de autenticación (tag) no parece estar presente explícitamente en el flujo original,
    // se asume que GCM la manejará internamente si está al final del ciphertext.
    // Si la desencriptación falla, puede que necesites extraerla por separado.
    ret = mbedtls_gcm_auth_decrypt(&gcm, ciphertext_len, iv, sizeof(iv), NULL,
                                   0,        // Sin datos adicionales de autenticación (AAD)
                                   NULL, 0,  // Asumiendo que el tag está al final del ciphertext
                                   ciphertext, decrypted_text);

    mbedtls_gcm_free(&gcm);

    if (ret != 0) {
      ESP_LOGE(TAG, "mbedtls_gcm_auth_decrypt failed: -0x%x", -ret);
      this->reset_telegram_();
      return;
    }
    // --- Fin del bloque de desencriptación con mbedTLS ---
