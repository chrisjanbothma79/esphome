  ...
  while (this->send_service_ < this->service_count_) {
    ...
    if (current_size + service_size > MAX_PACKET_SIZE) {
      ...
      if (resp.services.size() > 1) {
        ...
        ESP_LOGD(TAG, "[%d] [%s] Service %d is too large (%d bytes) but sending anyway", this->connection_index_,
    ...
    ESP_LOGD(TAG, "[%d] [%s] Total size now: %d", this->connection_index_, this->address_str().c_str(), current_size);
