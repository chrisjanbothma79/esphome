...
size_t ESP32CameraJPEGEncoder::encode_pixels(camera::CameraImageSpec *spec, camera::CameraImage *pixels) {
  this->bytes_written_ = 0;
  this->last_error_ = camera::ENCODER_ERROR_SUCCESS;
  this->should_expand_size_ = false;
  ...
  if (!success) {
    last_error_ = camera::ENCODER_ERROR_CONFIGURATION;
    return 0;
  }

  if (this->should_expand_size_) {
    ...
    if (this->output_->set_buffer_size(new_size)) {
      ...
      last_error_ = camera::ENCODER_ERROR_RETRY_FRAME;
    } else {
      ESP_LOGE(TAG, "Failed to expande output buffer.");
      this->buffer_expand_size_ = 0;
      last_error_ = camera::ENCODER_ERROR_SKIP_FRAME;
    }
...
size_t ESP32CameraJPEGEncoder::callback_(void *arg, size_t index, const void *data, size_t len) {
  ...
  if (index + len > buffer_length) {
    that->last_error_ = camera::ENCODER_ERROR_SKIP_FRAME;
    if (that->buffer_expand_size_ > 0)
      that->should_expand_size_ = true;
