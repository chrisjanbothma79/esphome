 ...
 * @return std::string
 */
std::string FsInterface::build_path(std::string path) {
  auto cur_path = path;
  if (!cur_path.starts_with("/")) {
    cur_path = mount_point + root_path + cur_path;
  } else {
    cur_path = mount_point + cur_path;
  }
  return cur_path;
}
...
bool FsInterface::is_exist(std::string path) {
  ...
  if (this->is_dir(cur_path)) {
    return true;
  }
  finfo info;
  rc = f_stat(cur_path.c_str(), &info);
...
bool FsInterface::mkdir(std::string name) {
  FRESULT rc = f_mkdir(build_path(name).c_str());
  return rc == FR_OK;
}
 ...
 * @return FsIterator
 */
FsIterator *FsInterface::list(std::string path) { return new FsIterator(build_path(path)); }

FileInterface *FsInterface::open_file(std::string path, const char mode) {
  FileInterface *file = new FileInterface(build_path(path).c_str(), mode);
  return file;
}
 ...
 * @param c
 */
void FileInterface::put(char c) { f_putc(c, &fp); }
...
FsIterator::FsIterator(std::string path) : path(path) { last_err = f_opendir(&dp, path.c_str()); }
