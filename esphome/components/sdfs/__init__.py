...
def _validate(config):
    if CORE.target_platform == PLATFORM_ESP8266:
        ...
        if config[CONF_TYPE] == "sdspi":
            if (CONF_BUS_SLOT not in config) and (
                config[CONF_MISO_PIN] not in config
                or config[CONF_MOSI_PIN] not in config
                or config[CONF_SCLK_PIN] not in config
            ):
                raise cv.Invalid(
                    "{CONF_BUS_SLOT}  or [{CONF_MOSI_PIN}, {CONF_MISO_PIN}, {CONF_SCLK_PIN}] required"
                )

        if config[CONF_TYPE] == "sdmmc":
...
async def to_code(config):
    ...
    if (CORE.using_esp_idf) and (config[CONF_TYPE] == "sdspi"):
        ...
        if CONF_INT_PIN in config:
            cg.add(var.set_int_pin(config[CONF_INT_PIN]))
