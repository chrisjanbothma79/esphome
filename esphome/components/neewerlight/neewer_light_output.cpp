...
bool NeewerRGBCTLightOutput::did_rgb_change(float red, float green, float blue) {
  if (red != this->old_red_)
    return true;
  if (green != this->old_green_)
    return true;
  if (blue != this->old_blue_)
    return true;
  return false;
};
...
bool NeewerRGBCTLightOutput::did_ctwb_change(float color_temperature, float white_brightness) {
  if (color_temperature != this->old_color_temperature_)
    return true;
  if (white_brightness != this->old_white_brightness_)
    return true;
  return false;
};
...
void NeewerRGBCTLightOutput::rgb_to_hsb(float red, float green, float blue, int *hue, uint8_t *saturation,
                                        uint8_t *brightness) {
  float max_value = 0.0;
  float min_value = 0.0;
