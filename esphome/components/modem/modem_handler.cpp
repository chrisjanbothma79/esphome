...
std::string ModemHandler::modem_network_status_string() {
  ...
  snprintf(buf, sizeof(buf),
           ...
           this->modem_connected ? "Good" : (this->synced ? "BAD" : "No SYNC"), this->network_attached ? "Yes" : "NO",
           cfun_str.c_str(), this->sim_status.c_str(), network_system_mode_to_string(this->network_mode).c_str(),
           this->ber, this->rssi, get_signal_bars(this->rssi).c_str());
  return {buf};
}
