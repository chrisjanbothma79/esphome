...
void PrometheusHandler::date_row_(AsyncResponseStream *stream, datetime::DateEntity *obj, std::string &area,
  ...
  if (obj->has_state()) {
    ...
    struct tm timeinfo_ = {};
    timeinfo_.tm_year = obj->year - 1900;
    timeinfo_.tm_mon = obj->month - 1;
    timeinfo_.tm_mday = obj->day;
    timeinfo_.tm_hour = 0;
    timeinfo_.tm_min = 0;
    timeinfo_.tm_sec = 0;
    time_t timestamp_ = mktime(&timeinfo_);
    stream->print(static_cast<int64_t>(timestamp_));
    stream->print(F("\n"));
  } else {
    // Invalid state
    stream->print(F("esphome_date_failed{id=\""));
...
void PrometheusHandler::datetime_row_(AsyncResponseStream *stream, datetime::DateTimeEntity *obj, std::string &area,
  ...
  if (obj->has_state()) {
    ...
    timeinfo_.tm_mon = obj->month - 1;
    timeinfo_.tm_mday = obj->day;
    timeinfo_.tm_hour = obj->hour;
    timeinfo_.tm_min = obj->minute;
    timeinfo_.tm_sec = obj->second;
    time_t timestamp_ = mktime(&timeinfo_);
    stream->print(static_cast<int64_t>(timestamp_));
    stream->print(F("\n"));
  } else {
