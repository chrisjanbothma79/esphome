...
void MIPI_DSI::setup() {
  ...
  esph_log_config(TAG, "MIPI_DSI setup complete");
}
...
void MIPI_DSI::write_command_(uint8_t value) {
  this->enable();
...
void MIPI_DSI::write_data_(uint8_t value) {
  this->enable();
  if (this->dc_pin_ == nullptr) {
...
void MIPI_DSI::dump_config() {
  ESP_LOGCONFIG("", "MIPI_DSI RGB LCD");
