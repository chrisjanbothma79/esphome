...
LD2410STriggerThresholdNumber = ld2410s_ns.class_("LD2410STriggerThresholdNumber", number.Number)
...
LD2410STriggerSelectedGateNumber = ld2410s_ns.class_("LD2410STriggerSelectedGateNumber", number.Number)
...
async def to_code(config):
    ...
    if trigger_threshold_config := config.get(CONF_TRIGGER_THRESHOLD):
        n = await number.new_number(trigger_threshold_config, min_value=10, max_value=95, step=1)
        await cg.register_parented(n, config[CONF_LD2410S_ID])
        cg.add(LD2410S_component.set_trigger_threshold_number(n))
    if trigger_hold_config := config.get(CONF_TRIGGER_HOLD):
        n = await number.new_number(trigger_hold_config, min_value=10, max_value=95, step=1)
        await cg.register_parented(n, config[CONF_LD2410S_ID])
        cg.add(LD2410S_component.set_trigger_hold_number(n))
    if trigger_snr_config := config.get(CONF_TRIGGER_SNR):
        n = await number.new_number(trigger_snr_config, min_value=5, max_value=63, step=1)
        await cg.register_parented(n, config[CONF_LD2410S_ID])
        cg.add(LD2410S_component.set_trigger_snr_number(n))
    if trigger_selected_gate_config := config.get(CONF_TRIGGER_SELECTED_GATE):
        n = await number.new_number(trigger_selected_gate_config, min_value=0, max_value=15, step=1)
        await cg.register_parented(n, config[CONF_LD2410S_ID])
