...
void DelonghiClimate::control(const climate::ClimateCall &call) {
  if (call.get_mode().has_value()) {
    ...
    if (this->mode == climate::CLIMATE_MODE_OFF) {
      ...
      this->mode = new_mode;
      this->action =
          (new_mode == climate::CLIMATE_MODE_COOL ? climate::CLIMATE_ACTION_COOLING : climate::CLIMATE_ACTION_IDLE);
      this->publish_state();
      return;
    }
    ...
    this->do_step_();
    return;
  } else if (call.get_preset().has_value() && *call.get_preset() == climate::CLIMATE_PRESET_NONE) {
    ...
    if (prev == "Silent")
      cmd = SILENT_COMMAND;
    else if (prev == "Eco Real Feel")
      cmd = ECO_REAL_FEEL_COMMAND;
...
void DelonghiClimate::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Temperature range: %u–%u °C (step %.1f)", DELONGHI_TEMP_MIN, DELONGHI_TEMP_MAX,
                this->traits().get_visual_current_temperature_step());
  ...
  ESP_LOGCONFIG(TAG, "    Temp up/down:       0x%04X / 0x%04X", DELONGHI_TEMP_UP, DELONGHI_TEMP_DOWN);
