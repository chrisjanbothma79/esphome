...
class DnsPythonWrapper:
    ...
    def check_record(self, name, query_type, expected=True, expect=None):
        ...
        if expect is None:
            expect = name
        if expected:
            assert any(expect in answer for answer in answers), (
                f"Expected record '{expect}' not found in answer section"
            )
        else:
            assert not any(expect in answer for answer in answers), (
