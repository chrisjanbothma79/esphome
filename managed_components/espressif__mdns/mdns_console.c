...
static void _udp_recv(void *arg, struct udp_pcb *upcb, struct pbuf *pb, const ip_addr_t *raddr, uint16_t rport) {
  ...
  while (pb != NULL) {
    ...
    // lwip does not return the proper pcb if you have more than one for the same multicast address (but different
    // interfaces)
    struct netif *netif = NULL;
    bool found = false;
...
#if LWIP_IPV4
        if (packet->src.type == IPADDR_TYPE_V4) {
          if ((packet->src.u_addr.ip4.addr & ip_2_ip4(&netif->netmask)->addr) !=
              (ip_2_ip4(&netif->ip_addr)->addr & ip_2_ip4(&netif->netmask)->addr)) {
...
esp_err_t _mdns_pcb_init(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol) {
  mdns_api_call_t msg = {.tcpip_if = tcpip_if, .ip_protocol = ip_protocol};
...
size_t _mdns_udp_pcb_write(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, const esp_ip_addr_t *ip, uint16_t port,
  ...
  mdns_api_call_t msg = {
      .tcpip_if = tcpip_if, .ip_protocol = ip_protocol, .pbt = pbt, .ip = &ip_add_copy, .port = port};
