...
#List of memory functions to prefix
    mem_funcs =["malloc", "calloc", "free", "strdup", "strndup"]

#Create regex pattern matching the memory functions but not already prefixed ones
                pattern = rf "(?<!{MDNS_MEM_PREFIX})(?<![\w])(" + "|".join(mem_funcs) + r ")(?=\s*\()"

#Replace all occurrences with configured prefix
                                                                           modified = re.sub(pattern, rf "{MDNS_MEM_PREFIX}\1", content)

                                                                                                 return modified

                                                                                             def process_file(filename) : try :
#Read the file
