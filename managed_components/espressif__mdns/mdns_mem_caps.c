...
enum interface_protocol { PROTO_IPV4 = 1 << MDNS_IP_PROTOCOL_V4, PROTO_IPV6 = 1 << MDNS_IP_PROTOCOL_V6 };
...
static void delete_socket(int sock) { close(sock); }
...
void *_mdns_get_packet_data(mdns_rx_packet_t *packet) { return packet->pb->payload; }

size_t _mdns_get_packet_len(mdns_rx_packet_t *packet) { return packet->pb->len; }
...
static inline size_t espaddr_to_inet(const esp_ip_addr_t *addr, const uint16_t port,
                                     const mdns_ip_protocol_t ip_protocol, struct sockaddr_storage *in_addr) {
...
size_t _mdns_udp_pcb_write(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, const esp_ip_addr_t *ip, uint16_t port,
    ...
    if (s < 0) {
      ...
      for (int tcpip_if = 0; tcpip_if < MDNS_MAX_INTERFACES; tcpip_if++) {
        ...
        if (FD_ISSET(sock, &rfds)) {
          ...
          int len = recvfrom(sock, recvbuf, sizeof(recvbuf), 0, (struct sockaddr *) &raddr, &socklen);
          if (len < 0) {
...
static int create_socket(esp_netif_t *netif) {
  ...
  if (ret < 0) {
    ESP_LOGE(TAG, "\"%s\" Unable to bind socket to specified interface. errno=%d: %s", esp_netif_get_desc(netif), errno,
             strerror(errno));
    goto err;
  }
