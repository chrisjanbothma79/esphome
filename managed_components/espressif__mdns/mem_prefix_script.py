...
#endif
#define MDNS_MAX_PREDEF_INTERFACES \
  (CONFIG_MDNS_PREDEF_NETIF_STA + CONFIG_MDNS_PREDEF_NETIF_AP + CONFIG_MDNS_PREDEF_NETIF_ETH)
...
#define HOOK_MALLOC_FAILED \
  ESP_LOGE(TAG, "Cannot allocate memory (line: %d, free heap: %" PRIu32 " bytes)", __LINE__, esp_get_free_heap_size());
#endif
...
typedef enum { MDNS_ANSWER, MDNS_NS, MDNS_EXTRA } mdns_parsed_record_type_t;
...
typedef enum { SEARCH_OFF, SEARCH_INIT, SEARCH_RUNNING, SEARCH_MAX } mdns_search_once_state_t;

typedef enum { BROWSE_OFF, BROWSE_INIT, BROWSE_RUNNING, BROWSE_MAX } mdns_browse_state_t;
