...
static mdns_search_once_t *_mdns_search_find_from(mdns_search_once_t *search, mdns_name_t *name, uint16_t type,
                                                  mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol);
static mdns_browse_t *_mdns_browse_find_from(mdns_browse_t *b, mdns_name_t *name, uint16_t type, mdns_if_t tcpip_if,
                                             mdns_ip_protocol_t ip_protocol);
static void _mdns_browse_result_add_srv(mdns_browse_t *browse, const char *hostname, const char *instance,
                                        const char *service, const char *proto, uint16_t port, mdns_if_t tcpip_if,
...
static void _mdns_browse_result_add_ip(mdns_browse_t *browse, const char *hostname, esp_ip_addr_t *ip,
                                       ...
                                       mdns_browse_sync_t *out_sync_browse);
static void _mdns_browse_result_add_txt(mdns_browse_t *browse, const char *instance, const char *service,
                                        const char *proto, mdns_txt_item_t *txt, uint8_t *txt_value_len,
...
static inline bool _str_null_or_empty(const char *str) { return (str == NULL || *str == 0); }
...
static const uint8_t *_mdns_read_fqdn(const uint8_t *packet, const uint8_t *start, mdns_name_t *name, char *buf,
  ...
  while (start + index < packet_end && start[index]) {
    ...
    if (len < 0xC0) {
      ...
      buf[len] = '\0';
      if (name->parts == 1 && buf[0] != '_' && (strcasecmp(buf, MDNS_DEFAULT_DOMAIN) != 0) &&
          (strcasecmp(buf, "arpa") != 0)
#ifndef CONFIG_MDNS_RESPOND_REVERSE_QUERIES
          && (strcasecmp(buf, "ip6") != 0) && (strcasecmp(buf, "in-addr") != 0)
#endif
      ) {
 ...
 * @return length of added data: 0 on error or length on success
 */
static uint16_t _mdns_append_ptr_record(uint8_t *packet, uint16_t *index, const char *instance, const char *service,
 ...
 * @return length of added data: 0 on error or length on success
 */
static uint16_t _mdns_append_sdptr_record(uint8_t *packet, uint16_t *index, mdns_service_t *service, bool flush,
                                          bool bye) {
  const char *str[3];
  const char *sd_str[4];
  uint16_t record_length = 0;
 ...
 * @return length of added data: 0 on error or length on success
 */
static uint16_t _mdns_append_txt_record(uint8_t *packet, uint16_t *index, mdns_service_t *service, bool flush,
                                        bool bye) {
  const char *str[4];
  uint16_t record_length = 0;
  uint8_t part_length;
 ...
 * @return length of added data: 0 on error or length on success
 */
static uint16_t _mdns_append_srv_record(uint8_t *packet, uint16_t *index, mdns_service_t *service, bool flush,
                                        bool bye) {
  const char *str[4];
  uint16_t record_length = 0;
  uint8_t part_length;
 ...
 * @return length of added data: 0 on error or length on success
 */
static uint16_t _mdns_append_a_record(uint8_t *packet, uint16_t *index, const char *hostname, uint32_t ip, bool flush,
                                      bool bye) {
  const char *str[2];
  uint16_t record_length = 0;
  uint8_t part_length;
 ...
 * @return length of added data: 0 on error or length on success
 */
static uint16_t _mdns_append_aaaa_record(uint8_t *packet, uint16_t *index, const char *hostname, uint8_t *ipv6,
                                         bool flush, bool bye) {
  const char *str[2];
  uint16_t record_length = 0;
  uint8_t part_length;
...
static uint8_t _mdns_append_host_answer(uint8_t *packet, uint16_t *index, mdns_host_item_t *host, uint8_t address_type,
                                        bool flush, bool bye) {
  ...
  while (addr != NULL) {
    if (addr->addr.type == address_type) {
      ...
      if (address_type == ESP_IPADDR_TYPE_V6 &&
          _mdns_append_aaaa_record(packet, index, host->hostname, (uint8_t *) addr->addr.u_addr.ip6.addr, flush, bye) <=
              0) {
        break;
      }
#endif /* CONFIG_LWIP_IPV6 */
...
static uint8_t _mdns_append_service_ptr_answers(uint8_t *packet, uint16_t *index, mdns_service_t *service, bool flush,
  ...
  if (_mdns_append_ptr_record(packet, index, _mdns_get_service_instance_name(service), service->service, service->proto,
                              flush, bye) <= 0) {
...
static uint8_t _mdns_append_answer(uint8_t *packet, uint16_t *index, mdns_out_answer_t *answer, mdns_if_t tcpip_if) {
  ...
  if (answer->type == MDNS_TYPE_PTR) {
    if (answer->service) {
      ...
      return _mdns_append_ptr_record(packet, index, answer->custom_instance, answer->custom_service,
                                     answer->custom_proto, answer->flush, answer->bye) > 0;
    }
    ...
    if (answer->host == &_mdns_self_host) {
      ...
      if (!mdns_is_netif_ready(tcpip_if, MDNS_IP_PROTOCOL_V4) &&
          _mdns_server->interfaces[tcpip_if].pcbs[MDNS_IP_PROTOCOL_V4].state != PCB_DUP) {
        return 0;
      }
      if (esp_netif_get_ip_info(_mdns_get_esp_netif(tcpip_if), &if_ip_info)) {
        return 0;
      }
      if (_mdns_append_a_record(packet, index, _mdns_server->hostname, if_ip_info.ip.addr, answer->flush,
                                answer->bye) <= 0) {
        return 0;
      }
      ...
      if (_mdns_append_a_record(packet, index, _mdns_server->hostname, if_ip_info.ip.addr, answer->flush, answer->bye) >
          0) {
        return 2;
      }
      return 1;
    ...
    if (answer->host == &_mdns_self_host) {
      ...
      uint8_t count = 0;
      if (!mdns_is_netif_ready(tcpip_if, MDNS_IP_PROTOCOL_V6) &&
          _mdns_server->interfaces[tcpip_if].pcbs[MDNS_IP_PROTOCOL_V6].state != PCB_DUP) {
        return 0;
      }
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (_mdns_append_aaaa_record(packet, index, _mdns_server->hostname, (uint8_t *) if_ip6s[i].addr, answer->flush,
                                     answer->bye) <= 0) {
          return 0;
        }
      }
      ...
      if (_mdns_append_aaaa_record(packet, index, _mdns_server->hostname, (uint8_t *) other_ip6.addr, answer->flush,
                                   answer->bye) > 0) {
        return 1 + count;
      }
      return count;
...
static void _mdns_clear_pcb_tx_queue_head(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol) {
  ...
  while (_mdns_server->tx_queue_head && _mdns_server->tx_queue_head->tcpip_if == tcpip_if &&
         _mdns_server->tx_queue_head->ip_protocol == ip_protocol) {
    q = _mdns_server->tx_queue_head;
    _mdns_server->tx_queue_head = _mdns_server->tx_queue_head->next;
    _mdns_free_tx_packet(q);
  }
 ...
 */
static void _mdns_remove_scheduled_answer(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, uint16_t type,
...
static bool _mdns_create_answer_from_service(mdns_tx_packet_t *packet, mdns_service_t *service,
  ...
  if (question->type == MDNS_TYPE_PTR || question->type == MDNS_TYPE_ANY) {
    ...
    if (!_mdns_alloc_answer(&packet->answers, MDNS_TYPE_PTR, service, NULL, false, false) ||
        !_mdns_alloc_answer(is_delegated ? &packet->additional : &packet->answers, MDNS_TYPE_SRV, service, NULL,
                            send_flush, false) ||
        !_mdns_alloc_answer(is_delegated ? &packet->additional : &packet->answers, MDNS_TYPE_TXT, service, NULL,
                            send_flush, false) ||
        !_mdns_alloc_answer((shared || is_delegated) ? &packet->additional : &packet->answers, MDNS_TYPE_A, service,
                            host, send_flush, false) ||
        !_mdns_alloc_answer((shared || is_delegated) ? &packet->additional : &packet->answers, MDNS_TYPE_AAAA, service,
                            host, send_flush, false)) {
      return false;
    }
        ...
        if (_mdns_service_match_ptr_question(service->service, q)) {
          ...
          while (r) {
            if (service->service->instance && r->host) {
              if (_mdns_service_match_instance(service->service, r->host, r->service, r->proto, NULL) &&
                  r->ttl > (MDNS_ANSWER_PTR_TTL / 2)) {
                is_record_exist = true;
                break;
              }
            } else if (!service->service->instance && !r->host) {
              if (_mdns_service_match(service->service, r->service, r->proto, NULL) &&
                  r->ttl > (MDNS_ANSWER_PTR_TTL / 2)) {
                is_record_exist = true;
                break;
              }
...
static bool _mdns_question_exists(mdns_out_question_t *needle, mdns_out_question_t *haystack) {
  while (haystack) {
    if (haystack->type == needle->type && haystack->host == needle->host && haystack->service == needle->service &&
 ...
 */
static mdns_tx_packet_t *_mdns_create_probe_packet(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol,
                                                   mdns_srv_item_t *services[], size_t len, bool first,
                                                   bool include_ip) {
 ...
 */
static mdns_tx_packet_t *_mdns_create_announce_packet(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol,
                                                      mdns_srv_item_t *services[], size_t len, bool include_ip) {
  mdns_tx_packet_t *packet = _mdns_alloc_packet_default(tcpip_if, ip_protocol);
  if (!packet) {
    return NULL;
  }
 ...
 */
static void _mdns_pcb_send_bye(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, mdns_srv_item_t **services,
 ...
 */
static void _mdns_init_pcb_probe_new_service(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol,
                                             mdns_srv_item_t **services, size_t len, bool probe_ip) {
  ...
  mdns_tx_packet_t *packet =
      _mdns_create_probe_packet(tcpip_if, ip_protocol, _services, services_final_len, true, probe_ip);
  if (!packet) {
 ...
 */
static void _mdns_init_pcb_probe(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, mdns_srv_item_t **services,
                                 size_t len, bool probe_ip) {
  mdns_pcb_t *pcb = &_mdns_server->interfaces[tcpip_if].pcbs[ip_protocol];
  ...
  if (PCB_STATE_IS_PROBING(pcb)) {
    ...
    _mdns_init_pcb_probe_new_service(tcpip_if, ip_protocol, new_probe_service_len ? new_probe_services : NULL,
 ...
 */
static void _mdns_send_bye_subtype(mdns_srv_item_t *service, const char *instance_name,
                                   mdns_subtype_t *remove_subtypes) {
  uint8_t i, j;
  for (i = 0; i < MDNS_MAX_INTERFACES; i++) {
    for (j = 0; j < MDNS_IP_PROTOCOL_MAX; j++) {
        ...
        while (a) {
          if (a->type == MDNS_TYPE_PTR && a->service) {
            ...
            while (current_subtype) {
              count += (_mdns_append_subtype_ptr_record(pkt, &index, instance_name, current_subtype->subtype,
                                                        a->service->service, a->service->proto, a->flush, a->bye) > 0);
 ...
 */
static void _mdns_announce_pcb(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, mdns_srv_item_t **services,
                               size_t len, bool include_ip) {
 ...
 */
static mdns_service_t *_mdns_create_service(const char *service, const char *proto, const char *hostname, uint16_t port,
                                            const char *instance, size_t num_items, mdns_txt_item_t txt[]) {
      ...
      if (PCB_STATE_IS_PROBING(_pcb)) {
        ...
        if (i < _pcb->probe_services_len) {
          ...
          if (q->questions) {
            ...
            if (qs->type == MDNS_TYPE_ANY && qs->service && strcmp(qs->service, service->service) == 0 && qs->proto &&
                strcmp(qs->proto, service->proto) == 0) {
              q->questions = q->questions->next;
              mdns_mem_free(qs);
            } else
              while (qs->next) {
                qsn = qs->next;
                if (qsn->type == MDNS_TYPE_ANY && qsn->service && strcmp(qsn->service, service->service) == 0 &&
 ...
 */
static int _mdns_check_srv_collision(mdns_service_t *service, uint16_t priority, uint16_t weight, uint16_t port,
...
static bool _hostname_is_ours(const char *hostname) {
  if (!_str_null_or_empty(_mdns_server->hostname) && strcasecmp(hostname, _mdns_server->hostname) == 0) {
    return true;
  }
  mdns_host_item_t *host = _mdns_host_list;
  while (host != NULL) {
...
static bool _mdns_name_is_discovery(mdns_name_t *name, uint16_t type) {
  return ((name->host[0] && !strcasecmp(name->host, "_services")) &&
          (name->service[0] && !strcasecmp(name->service, "_dns-sd")) &&
          (name->proto[0] && !strcasecmp(name->proto, "_udp")) &&
          (name->domain[0] && !strcasecmp(name->domain, MDNS_DEFAULT_DOMAIN)) && type == MDNS_TYPE_PTR);
}
...
static bool _mdns_name_is_ours(mdns_name_t *name) {
  ...
  if (_str_null_or_empty(name->service) && _str_null_or_empty(name->proto)) {
    if (!_str_null_or_empty(name->host) && !_str_null_or_empty(_mdns_server->hostname) &&
...
static inline uint32_t _mdns_read_u32(const uint8_t *packet, uint16_t index) {
  return (uint32_t) (packet[index]) << 24 | (uint32_t) (packet[index + 1]) << 16 | (uint32_t) (packet[index + 2]) << 8 |
         packet[index + 3];
}
...
static bool _mdns_question_matches(mdns_parsed_question_t *question, uint16_t type, mdns_srv_item_t *service) {
  ...
  if (type == MDNS_TYPE_A || type == MDNS_TYPE_AAAA) {
    ...
    if (question->service && question->proto && question->domain &&
        ...
        !strcasecmp(service->service->proto, question->proto) && !strcasecmp(MDNS_DEFAULT_DOMAIN, question->domain)) {
      if (!service->service->instance) {
        return true;
      } else if (service->service->instance && question->host &&
                 !strcasecmp(service->service->instance, question->host)) {
        return true;
      }
    }
    ...
    if (name && question->host && question->service && question->proto && question->domain &&
        !strcasecmp(name, question->host) && !strcasecmp(service->service->service, question->service) &&
        !strcasecmp(service->service->proto, question->proto) && !strcasecmp(MDNS_DEFAULT_DOMAIN, question->domain)) {
      return true;
    }
  }
...
void mdns_parse_packet(mdns_rx_packet_t *packet) {
  ...
  if (packet->src.type == ESP_IPADDR_TYPE_V4) {
    _mdns_dbg_printf("From: " IPSTR ":%u, To: " IPSTR ", ", IP2STR(&packet->src.u_addr.ip4), packet->src_port,
                     IP2STR(&packet->dest.u_addr.ip4));
  }
#endif
#ifdef CONFIG_LWIP_IPV6
  if (packet->src.type == ESP_IPADDR_TYPE_V6) {
    _mdns_dbg_printf("From: " IPV6STR ":%u, To: " IPV6STR ", ", IPV62STR(packet->src.u_addr.ip6), packet->src_port,
                     IPV62STR(packet->dest.u_addr.ip6));
  }
#endif
  ...
  if (header.questions) {
    ...
    while (qs--) {
      ...
      if (_mdns_name_is_discovery(name, type)) {
        ...
        mdns_srv_item_t *a = _mdns_server->services;
        while (a) {
          mdns_parsed_question_t *question =
              (mdns_parsed_question_t *) mdns_mem_calloc(1, sizeof(mdns_parsed_question_t));
          if (!question) {
            HOOK_MALLOC_FAILED;
  ...
  if (header.questions && !parsed_packet->questions && !parsed_packet->discovery && !header.answers) {
    ...
    while (content < (data + len)) {
      ...
      if (parsed_packet->discovery && _mdns_name_is_discovery(name, type)) {
        ...
        search_result =
            _mdns_search_find_from(_mdns_server->search_once, name, type, packet->tcpip_if, packet->ip_protocol);
      ...
      if (type == MDNS_TYPE_PTR) {
        ...
        if (search_result) {
          _mdns_search_result_add_ptr(search_result, name->host, name->service, name->proto, packet->tcpip_if,
        ...
        if (search_result && search_result->type == MDNS_TYPE_PTR) {
          ...
          while (result) {
            if (_mdns_get_esp_netif(packet->tcpip_if) == result->esp_netif &&
                packet->ip_protocol == result->ip_protocol && result->instance_name &&
                !strcmp(name->host, result->instance_name)) {
              break;
            }
            result = result->next;
          }
        ...
        if (search_result) {
          ...
          } else if (ttl > 60 && !col && !parsed_packet->authoritative && !parsed_packet->probe &&
        ...
        if (browse_result) {
          ...
          _mdns_browse_result_add_txt(browse_result, browse_result_instance, browse_result_service, browse_result_proto,
                                      txt, txt_value_len, txt_count, packet->tcpip_if, packet->ip_protocol, ttl,
                                      out_sync_browse);
        }
        if (search_result) {
          if (search_result->type == MDNS_TYPE_PTR) {
            ...
            _mdns_result_txt_create(data_ptr, data_len, &txt, &txt_value_len, &txt_count);
            if (txt_count) {
              _mdns_search_result_add_txt(search_result, txt, txt_value_len, txt_count, packet->tcpip_if,
                                          packet->ip_protocol, ttl);
            }
          }
        } else if (ours) {
          ...
          } else if (ttl > (MDNS_ANSWER_TXT_TTL / 2) && !col && !parsed_packet->authoritative &&
                     !parsed_packet->probe && !parsed_packet->questions &&
        ...
        if (browse_result) {
          _mdns_browse_result_add_ip(browse_result, name->host, &ip6, packet->tcpip_if, packet->ip_protocol, ttl,
                                     out_sync_browse);
        }
        if (search_result) {
          ...
          while (search_result) {
            ...
            search_result =
                _mdns_search_find_from(search_result->next, name, type, packet->tcpip_if, packet->ip_protocol);
          }
        } else if (ours) {
          if (parsed_packet->questions && !parsed_packet->probe) {
        ...
        if (browse_result) {
          _mdns_browse_result_add_ip(browse_result, name->host, &ip, packet->tcpip_if, packet->ip_protocol, ttl,
                                     out_sync_browse);
        }
        if (search_result) {
          // check for more applicable searches (PTR & A/AAAA at the same time)
  ...
  if (!do_not_reply && _mdns_server->interfaces[packet->tcpip_if].pcbs[packet->ip_protocol].state > PCB_PROBE_3 &&
      (parsed_packet->questions || parsed_packet->discovery)) {
...
#ifdef CONFIG_MDNS_RESPOND_REVERSE_QUERIES
static inline char nibble_to_hex(int var) { return var > 9 ? var - 10 + 'a' : var + '0'; }
#endif
...
static void perform_event_action(mdns_if_t mdns_if, mdns_event_actions_t action) {
  ...
  if (action & MDNS_EVENT_IP4_REVERSE_LOOKUP) {
    ...
    if (esp_netif_get_ip_info(_mdns_get_esp_netif(mdns_if), &if_ip_info) == ESP_OK) {
      ...
      if (asprintf(&reverse_query_name, "%d.%d.%d.%d.in-addr", esp_ip4_addr4_16(ip), esp_ip4_addr3_16(ip),
                   esp_ip4_addr2_16(ip), esp_ip4_addr1_16(ip)) > 0 &&
          reverse_query_name) {
  ...
  if (action & MDNS_EVENT_IP6_REVERSE_LOOKUP) {
    ...
    if (!esp_netif_get_ip6_linklocal(_mdns_get_esp_netif(mdns_if), &addr6) && !_ipv6_address_is_zero(addr6)) {
      ...
      const char sub[] = "ip6";
      const size_t query_name_size =
          4 * sizeof(addr6.addr) /* (2 nibbles + 2 dots)/per byte of IP address */ + sizeof(sub);
...
static inline void post_mdns_disable_pcb(mdns_predef_if_t preset_if, mdns_ip_protocol_t protocol) {
  mdns_post_custom_action_tcpip_if(mdns_if_from_preset_if(preset_if),
...
void mdns_preset_if_handle_system_event(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
  if (!_mdns_server) {
    return;
  }
...
static mdns_search_once_t *_mdns_search_init(const char *name, const char *service, const char *proto, uint16_t type,
                                             bool unicast, uint32_t timeout, uint8_t max_results,
...
static inline void _mdns_result_update_ttl(mdns_result_t *r, uint32_t ttl) { r->ttl = r->ttl < ttl ? r->ttl : ttl; }
...
static void _mdns_search_result_add_ip(mdns_search_once_t *search, const char *hostname, esp_ip_addr_t *ip,
  ...
  if ((search->type == MDNS_TYPE_A && ip->type == ESP_IPADDR_TYPE_V4) ||
      (search->type == MDNS_TYPE_AAAA && ip->type == ESP_IPADDR_TYPE_V6) || search->type == MDNS_TYPE_ANY) {
    r = search->result;
    while (r) {
    ...
    while (r) {
      if (r->esp_netif == _mdns_get_esp_netif(tcpip_if) && r->ip_protocol == ip_protocol &&
          !_str_null_or_empty(r->hostname) && !strcasecmp(hostname, r->hostname)) {
        _mdns_result_add_ip(r, ip);
        _mdns_result_update_ttl(r, ttl);
        break;
      }
 ...
 */
static mdns_search_once_t *_mdns_search_find_from(mdns_search_once_t *s, mdns_name_t *name, uint16_t type,
                                                  mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol) {
  mdns_result_t *r = NULL;
  while (s) {
    ...
    if (type == MDNS_TYPE_A || type == MDNS_TYPE_AAAA) {
      ...
      while (r) {
        if (r->esp_netif == _mdns_get_esp_netif(tcpip_if) && r->ip_protocol == ip_protocol &&
            !_str_null_or_empty(r->hostname) && !strcasecmp(name->host, r->hostname)) {
          return s;
        }
        r = r->next;
      }
    ...
    if (type == MDNS_TYPE_SRV || type == MDNS_TYPE_TXT) {
      ...
      if (strcasecmp(name->service, s->service) || strcasecmp(name->proto, s->proto)) {
        s = s->next;
        continue;
      }
    ...
    if (type == MDNS_TYPE_PTR && type == s->type && !strcasecmp(name->service, s->service) &&
 ...
 */
static mdns_tx_packet_t *_mdns_create_search_packet(mdns_search_once_t *search, mdns_if_t tcpip_if,
                                                    mdns_ip_protocol_t ip_protocol) {
  mdns_result_t *r = NULL;
  mdns_tx_packet_t *packet = _mdns_alloc_packet_default(tcpip_if, ip_protocol);
  if (!packet) {
  ...
  if (search->type == MDNS_TYPE_PTR) {
    ...
    while (r) {
      ...
      if (r->esp_netif != _mdns_get_esp_netif(tcpip_if) || r->ip_protocol != ip_protocol || r->instance_name == NULL ||
          r->hostname == NULL || r->addr == NULL) {
        r = r->next;
        continue;
      }
      mdns_out_answer_t *a = (mdns_out_answer_t *) mdns_mem_malloc(sizeof(mdns_out_answer_t));
...
static void _mdns_search_send_pcb(mdns_search_once_t *search, mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol) {
  ...
  if (mdns_is_netif_ready(tcpip_if, ip_protocol) &&
      _mdns_server->interfaces[tcpip_if].pcbs[ip_protocol].state > PCB_INIT) {
...
static void _mdns_remap_self_service_hostname(const char *old_hostname, const char *new_hostname) {
  ...
  while (service) {
    if (service->service->hostname && strcmp(service->service->hostname, old_hostname) == 0) {
...
static esp_err_t _mdns_start_timer(void) {
  esp_timer_create_args_t timer_conf = {
      .callback = _mdns_timer_cb, .arg = NULL, .dispatch_method = ESP_TIMER_TASK, .name = "mdns_timer"};
...
static esp_err_t _mdns_task_create_with_caps(void) {
  ...
  ESP_GOTO_ON_FALSE(_mdns_stack_buffer != NULL, ESP_FAIL, err, TAG,
                    "failed to allocate memory for the mDNS task's stack");

  _mdns_service_task_handle =
      xTaskCreateStaticPinnedToCore(_mdns_service_task, "mdns", MDNS_SERVICE_STACK_DEPTH, NULL, MDNS_TASK_PRIORITY,
                                    _mdns_stack_buffer, &mdns_task_buffer, MDNS_TASK_AFFINITY);
...
#if MDNS_ESP_WIFI_ENABLED && (CONFIG_MDNS_PREDEF_NETIF_STA || CONFIG_MDNS_PREDEF_NETIF_AP)
  if ((err = esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, mdns_preset_if_handle_system_event, NULL)) !=
      ESP_OK) {
    goto free_event_handlers;
  }
#endif
#if CONFIG_MDNS_PREDEF_NETIF_STA || CONFIG_MDNS_PREDEF_NETIF_AP || CONFIG_MDNS_PREDEF_NETIF_ETH
  if ((err = esp_event_handler_register(IP_EVENT, ESP_EVENT_ANY_ID, mdns_preset_if_handle_system_event, NULL)) !=
      ESP_OK) {
    goto free_event_handlers;
  }
#endif
#if CONFIG_ETH_ENABLED && CONFIG_MDNS_PREDEF_NETIF_ETH
  if ((err = esp_event_handler_register(ETH_EVENT, ESP_EVENT_ANY_ID, mdns_preset_if_handle_system_event, NULL)) !=
      ESP_OK) {
    goto free_event_handlers;
  }
#endif
...
esp_err_t mdns_service_add_for_host(const char *instance, const char *service, const char *proto, const char *host,
  ...
  ESP_GOTO_ON_FALSE(_mdns_can_add_more_services(), ESP_ERR_NO_MEM, err, TAG,
                    "Cannot add more services, please increase CONFIG_MDNS_MAX_SERVICES (%d)",
...
static mdns_txt_item_t *_copy_mdns_txt_items(mdns_txt_linked_item_t *items, uint8_t **txt_value_len,
                                             size_t *txt_count) {
...
static mdns_result_t *_mdns_lookup_service(const char *instance, const char *service, const char *proto,
  ...
  while (s) {
    ...
    bool is_service_selfhosted =
        !_str_null_or_empty(_mdns_server->hostname) && !strcasecmp(_mdns_server->hostname, srv->hostname);
    bool is_service_delegated =
        _str_null_or_empty(_mdns_server->hostname) || strcasecmp(_mdns_server->hostname, srv->hostname);
    if ((selfhost && is_service_selfhosted) || (!selfhost && is_service_delegated)) {
      if (!strcasecmp(srv->service, service) && !strcasecmp(srv->proto, proto) &&
        ...
        // We should not append addresses for selfhost lookup result as we don't know which interface's address to
        // append.
        if (selfhost) {
          item->addr = NULL;
        } else {
...
esp_err_t mdns_service_port_set_for_host(const char *instance, const char *service, const char *proto, const char *host,
                                         uint16_t port) {
  MDNS_SERVICE_LOCK();
  esp_err_t ret = ESP_OK;
  const char *hostname = host ? host : _mdns_server->hostname;
  ESP_GOTO_ON_FALSE(
      _mdns_server && _mdns_server->services && !_str_null_or_empty(service) && !_str_null_or_empty(proto) && port,
...
esp_err_t mdns_service_txt_set_for_host(const char *instance, const char *service, const char *proto, const char *host,
  ...
  esp_err_t ret = ESP_OK;
  const char *hostname = host ? host : _mdns_server->hostname;
  ESP_GOTO_ON_FALSE(_mdns_server && _mdns_server->services && !_str_null_or_empty(service) &&
                        !_str_null_or_empty(proto) && !(num_items && txt_items == NULL),
                    ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
  mdns_srv_item_t *s = _mdns_get_service_item_instance(instance, service, proto, hostname);
  ESP_GOTO_ON_FALSE(s, ESP_ERR_NOT_FOUND, err, TAG, "Service doesn't exist");
...
esp_err_t mdns_service_txt_item_set_for_host_with_explicit_value_len(const char *instance, const char *service,
                                                                     const char *proto, const char *host,
                                                                     const char *key, const char *value_arg,
                                                                     uint8_t value_len) {
  MDNS_SERVICE_LOCK();
  esp_err_t ret = ESP_OK;
  char *value = NULL;
  ...
  const char *hostname = host ? host : _mdns_server->hostname;
  ESP_GOTO_ON_FALSE(_mdns_server && _mdns_server->services && !_str_null_or_empty(service) &&
                        !_str_null_or_empty(proto) && !_str_null_or_empty(key) && !((!value_arg && value_len)),
                    ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
...
esp_err_t mdns_service_txt_item_set_for_host(const char *instance, const char *service, const char *proto,
                                             const char *hostname, const char *key, const char *value) {
...
esp_err_t mdns_service_txt_item_set(const char *service, const char *proto, const char *key, const char *value) {
  ...
  return mdns_service_txt_item_set_for_host_with_explicit_value_len(NULL, service, proto, NULL, key, value,
                                                                    strlen(value));
}
...
esp_err_t mdns_service_txt_item_remove_for_host(const char *instance, const char *service, const char *proto,
                                                const char *host, const char *key) {
  MDNS_SERVICE_LOCK();
  esp_err_t ret = ESP_OK;
  const char *hostname = host ? host : _mdns_server->hostname;
  ESP_GOTO_ON_FALSE(_mdns_server && _mdns_server->services && !_str_null_or_empty(service) &&
                        !_str_null_or_empty(proto) && !_str_null_or_empty(key),
                    ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
...
esp_err_t mdns_service_subtype_remove_for_host(const char *instance_name, const char *service, const char *proto,
  ...
  esp_err_t ret = ESP_OK;
  ESP_GOTO_ON_FALSE(_mdns_server && _mdns_server->services && !_str_null_or_empty(service) &&
                        !_str_null_or_empty(proto) && !_str_null_or_empty(subtype),
                    ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
...
static esp_err_t _mdns_service_subtype_add_for_host(mdns_srv_item_t *service, const char *subtype) {
  ...
  mdns_subtype_t *srv_subtype = service->service->subtype;
  while (srv_subtype) {
    ESP_GOTO_ON_FALSE(strcmp(srv_subtype->subtype, subtype) != 0, ESP_ERR_INVALID_ARG, err, TAG,
                      "Subtype: %s has already been added", subtype);
    srv_subtype = srv_subtype->next;
  }
...
esp_err_t mdns_service_subtype_add_multiple_items_for_host(const char *instance_name, const char *service,
                                                           const char *proto, const char *hostname,
                                                           mdns_subtype_item_t subtype[], uint8_t num_items) {
  MDNS_SERVICE_LOCK();
  esp_err_t ret = ESP_OK;
  int cur_index = 0;
  ESP_GOTO_ON_FALSE(_mdns_server && _mdns_server->services && !_str_null_or_empty(service) &&
                        !_str_null_or_empty(proto) && (num_items > 0),
                    ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
...
esp_err_t mdns_service_subtype_update_multiple_items_for_host(const char *instance_name, const char *service_type,
                                                              const char *proto, const char *hostname,
                                                              mdns_subtype_item_t subtype[], uint8_t num_items) {
  MDNS_SERVICE_LOCK();
  esp_err_t ret = ESP_OK;
  int cur_index = 0;
  ESP_GOTO_ON_FALSE(
      _mdns_server && _mdns_server->services && !_str_null_or_empty(service_type) && !_str_null_or_empty(proto),
      ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
...
esp_err_t mdns_service_instance_name_set_for_host(const char *instance_old, const char *service, const char *proto,
                                                  const char *host, const char *instance) {
  MDNS_SERVICE_LOCK();
  esp_err_t ret = ESP_OK;
  const char *hostname = host ? host : _mdns_server->hostname;

  ESP_GOTO_ON_FALSE(_mdns_server && _mdns_server->services && !_str_null_or_empty(service) &&
                        !_str_null_or_empty(proto) && !_str_null_or_empty(instance) &&
                        strlen(instance) <= (MDNS_NAME_BUF_LEN - 1),
                    ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
...
esp_err_t mdns_service_remove_for_host(const char *instance, const char *service, const char *proto, const char *host) {
  ...
  const char *hostname = host ? host : _mdns_server->hostname;
  ESP_GOTO_ON_FALSE(
      _mdns_server && _mdns_server->services && !_str_null_or_empty(service) && !_str_null_or_empty(proto),
      ESP_ERR_INVALID_ARG, err, TAG, "Invalid state or arguments");
  mdns_srv_item_t *s = _mdns_get_service_item_instance(instance, service, proto, hostname);
  ESP_GOTO_ON_FALSE(s, ESP_ERR_NOT_FOUND, err, TAG, "Service doesn't exist");
...
esp_err_t mdns_query_generic(const char *name, const char *service, const char *proto, uint16_t type,
                             mdns_query_transmission_type_t transmission_type, uint32_t timeout, size_t max_results,
  ...
  *results = NULL;
  ...
  search = _mdns_search_init(name, service, proto, type, transmission_type == MDNS_QUERY_UNICAST, timeout, max_results,
                             NULL);
  if (!search) {
    return ESP_ERR_NO_MEM;
  }
...
esp_err_t mdns_query_ptr(const char *service, const char *proto, uint32_t timeout, size_t max_results,
                         mdns_result_t **results) {
  if (_str_null_or_empty(service) || _str_null_or_empty(proto)) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t mdns_query_srv(const char *instance, const char *service, const char *proto, uint32_t timeout,
...
esp_err_t mdns_query_txt(const char *instance, const char *service, const char *proto, uint32_t timeout,
                         mdns_result_t **result) {
  if (_str_null_or_empty(instance) || _str_null_or_empty(service) || _str_null_or_empty(proto)) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t mdns_lookup_delegated_service(const char *instance, const char *service, const char *proto,
                                        size_t max_results, mdns_result_t **result) {
  if (!_mdns_server) {
    return ESP_ERR_INVALID_STATE;
  }
...
esp_err_t mdns_lookup_selfhosted_service(const char *instance, const char *service, const char *proto,
                                         size_t max_results, mdns_result_t **result) {
  if (!_mdns_server) {
    return ESP_ERR_INVALID_STATE;
  }
...
void mdns_debug_packet(const uint8_t *data, size_t len) {
  ...
  _mdns_dbg_printf("%s", (header.flags == MDNS_FLAGS_QR_AUTHORITATIVE) ? "AUTHORITATIVE\n"
  ...
  if (header.questions) {
    ...
    while (qs--) {
      ...
      if (type == MDNS_TYPE_PTR) {
        _mdns_dbg_printf("%s.%s%s.%s.%s. PTR ", name->host, name->sub ? "_sub." : "", name->service, name->proto,
                         name->domain);
      } else if (type == MDNS_TYPE_SRV) {
        _mdns_dbg_printf("%s.%s%s.%s.%s. SRV ", name->host, name->sub ? "_sub." : "", name->service, name->proto,
                         name->domain);
      } else if (type == MDNS_TYPE_TXT) {
        _mdns_dbg_printf("%s.%s%s.%s.%s. TXT ", name->host, name->sub ? "_sub." : "", name->service, name->proto,
                         name->domain);
      } else if (type == MDNS_TYPE_A) {
        ...
        _mdns_dbg_printf("%s.%s%s.%s.%s. NSEC ", name->host, name->sub ? "_sub." : "", name->service, name->proto,
                         name->domain);
      } else if (type == MDNS_TYPE_ANY) {
        _mdns_dbg_printf("%s.%s%s.%s.%s. ANY ", name->host, name->sub ? "_sub." : "", name->service, name->proto,
                         name->domain);
      } else {
        _mdns_dbg_printf("%s.%s%s.%s.%s. %04X ", name->host, name->sub ? "_sub." : "", name->service, name->proto,
                         name->domain, type);
      }
  ...
  if (header.answers || header.servers || header.additional) {
    ...
    while (content < (data + len)) {
      ...
      if (type == MDNS_TYPE_PTR) {
        _mdns_dbg_printf(": %s%s%s.%s.%s. PTR ", name->host, name->host[0] ? "." : "", name->service, name->proto,
                         name->domain);
      } else if (type == MDNS_TYPE_SRV) {
...
static void _mdns_browse_add(mdns_browse_t *browse) {
  ...
  while (queue) {
    if (strlen(queue->service) == strlen(browse->service) &&
        memcmp(queue->service, browse->service, strlen(queue->service)) == 0 &&
        strlen(queue->proto) == strlen(browse->proto) &&
        memcmp(queue->proto, browse->proto, strlen(queue->proto)) == 0) {
      found = true;
      break;
    }
    queue = queue->next;
  ...
  if (browse) {
    ...
    while (r) {
      if (r->ip_protocol == ip_protocol) {
        ...
        if (r->esp_netif == _mdns_get_esp_netif(tcpip_if) && !_str_null_or_empty(r->hostname) &&
            !strcasecmp(hostname, r->hostname)) {
          r_a = r->addr;
          // Check if the address has already added in result.
          while (r_a) {
#ifdef CONFIG_LWIP_IPV4
            if (r_a->addr.type == ip->type && r_a->addr.type == ESP_IPADDR_TYPE_V4 &&
                r_a->addr.u_addr.ip4.addr == ip->u_addr.ip4.addr) {
              break;
            }
#endif
#ifdef CONFIG_LWIP_IPV6
            if (r_a->addr.type == ip->type && r_a->addr.type == ESP_IPADDR_TYPE_V6 &&
                !memcmp(r_a->addr.u_addr.ip6.addr, ip->u_addr.ip6.addr, 16)) {
              break;
            }
#endif
            r_a = r_a->next;
...
static bool is_txt_item_in_list(mdns_txt_item_t txt, uint8_t txt_value_len, mdns_txt_item_t *txt_list,
                                uint8_t *txt_value_len_list, size_t txt_count) {
  for (size_t i = 0; i < txt_count; i++) {
      ...
      if (r->txt) {
        ...
        if (txt_count != r->txt_count) {
          ...
          for (size_t txt_index = 0; txt_index < txt_count; txt_index++) {
            if (!is_txt_item_in_list(txt[txt_index], txt_value_len[txt_index], r->txt, r->txt_value_len,
                                     r->txt_count)) {
              should_update = true;
              break;
            }
...
static esp_err_t _mdns_copy_address_in_previous_result(mdns_result_t *result_list, mdns_result_t *r) {
  while (result_list) {
    if (!_str_null_or_empty(result_list->hostname) && !_str_null_or_empty(r->hostname) &&
        !strcasecmp(result_list->hostname, r->hostname) && result_list->ip_protocol == r->ip_protocol &&
        result_list->addr && !r->addr) {
