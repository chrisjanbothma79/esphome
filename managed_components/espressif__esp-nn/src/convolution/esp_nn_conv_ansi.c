...
__attribute__((noinline)) static void esp_nn_conv_s8_1x1(const data_dims_t *input_dims, const int8_t *input_data,
                                                         ...
                                                         const quant_data_t *quant_data, void *scratch) {
...
__attribute__((noinline)) static void esp_nn_conv_s8_padded(const data_dims_t *input_dims, const int8_t *input_data,
                                                            const data_dims_t *filter_dims, const int8_t *filter_data,
                                                            const int32_t *bias, const data_dims_t *output_dims,
                                                            int8_t *out_data, const conv_params_t *conv_params,
                                                            const quant_data_t *quant_data, void *scratch) {
  const uint16_t input_wd = input_dims->width;
    ...
    for (int32_t out_x = 0; out_x < out_wd; out_x++) {
      ...
      for (int32_t out_ch_idx = 0; out_ch_idx < out_channels; out_ch_idx++) {
        ...
        if (row_size >= 16) {
          ...
          asm volatile("esp.movx.r.xacc.l  %0          \n\t" : "=r"(conv_out_asm));
...
int esp_nn_get_conv_scratch_size_esp32p4(const data_dims_t *input_dims, const data_dims_t *filter_dims,
...
void esp_nn_conv_s8_esp32p4(const data_dims_t *input_dims, const int8_t *input, const data_dims_t *filter_dims,
  ...
  if (filter_wd == 1 && filter_ht == 1 && pad_wd == 0 && pad_ht == 0 && stride_wd == 1 && stride_ht == 1) {
    esp_nn_conv_s8_1x1(input_dims, input, filter_data, bias, output_dims, out_data, conv_params, quant_data,
                       scratch_buffer);
  } else if (pad_wd == 0 && pad_ht == 0) {
    esp_nn_conv_s8_padded(input_dims, input, filter_dims, filter_data, bias, output_dims, out_data, conv_params,
                          quant_data, scratch_buffer);
  } else {
    esp_nn_conv_s8_opt(input_dims, input, filter_dims, filter_data, bias, output_dims, out_data, conv_params,
                       quant_data);
  }
}
