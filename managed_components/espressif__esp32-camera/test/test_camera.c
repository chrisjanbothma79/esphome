...
#if defined(CONFIG_MDNS_ADD_CUSTOM_NETIF) && !defined(CONFIG_MDNS_PREDEF_NETIF_STA) && \
...
static void query_mdns_host_with_getaddrinfo(char *host) {
  ...
  if (!getaddrinfo(host, NULL, &hints, &res)) {
    while (res) {
      char *resolved_addr;
#if defined(CONFIG_LWIP_IPV6) && defined(CONFIG_LWIP_IPV4)
      resolved_addr = res->ai_family == AF_INET ? inet_ntoa(((struct sockaddr_in *) res->ai_addr)->sin_addr)
