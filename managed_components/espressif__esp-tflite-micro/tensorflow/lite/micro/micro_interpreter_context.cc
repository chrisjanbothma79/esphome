...
class MicroInterpreterContext : public MicroContext {
 public:
  ...
  int AllocateDecompressionScratchBuffer(const TfLiteNode *node, int tensor_idx) override;

  // Available during Prepare & Eval. Returns nullptr if tensor is not
  // compressed.
  const CompressionTensorData *GetTensorCompressionData(const TfLiteNode *node, int tensor_idx) override;

  // Only available during Prepare & Eval. Returns nullptr on failure, otherwise
  // returns a pointer to the buffer.
  void *DecompressTensorToBuffer(const TfLiteEvalTensor &tensor, const CompressionTensorData &compression_data,
                                 void *buffer) override;

  // Set the alternate decompression memory regions.
  // Can only be called during the MicroInterpreter kInit state.
  TfLiteStatus SetDecompressionMemory(const std::initializer_list<AlternateMemoryRegion> &regions) override;
  ...
  TfLiteStatus SetAlternateProfiler(MicroProfilerInterface *alt_profiler) override;
 ...
 private:
  ...
  const std::initializer_list<AlternateMemoryRegion> *decompress_regions_ = nullptr;
