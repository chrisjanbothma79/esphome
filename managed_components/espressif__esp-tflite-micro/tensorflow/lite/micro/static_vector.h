...
template<typename T> void BroadcastAdd(const T input_scalar, const T *weights, T *output, const size_t count) {
...
const TFLMRegistration *BroadcastAddOp::getRegistration() { return GetMutableRegistration(); }
...
void *BroadcastAddOp::Init(TfLiteContext *context, const char *buffer, size_t length) {
...
TfLiteStatus BroadcastAddOp::Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TF_LITE_ENSURE(context, input->type == kTfLiteFloat32 || input->type == kTfLiteInt8 || input->type == kTfLiteInt16 ||
  ...
  TF_LITE_ENSURE(context, ElementCount(*weights->dims) == ElementCount(*output->dims));
  ...
  weight_scratch_index_ = micro_context->AllocateDecompressionScratchBuffer(node, 1);
...
TfLiteStatus BroadcastAddOp::Invoke(TfLiteContext *context, TfLiteNode *node) {
  ...
  const CompressionTensorData *weights_comp_td = micro_context->GetTensorCompressionData(node, 1);
  if (micro_context->IsTensorCompressed(node, 1)) {
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
      BroadcastAdd(tflite::micro::GetTensorData<float>(input)[0],
#ifdef USE_TFLM_COMPRESSION
                   tflite::micro::GetTensorData<float>(micro_context, weights, weights_comp_td, weight_scratch_index_),
#else   // USE_TFLM_COMPRESSION
                   tflite::micro::GetTensorData<float>(weights),
#endif  // USE_TFLM_COMPRESSION
                   tflite::micro::GetTensorData<float>(output), ElementCount(*output->dims));
    } break;

    case kTfLiteInt8: {
      BroadcastAdd(tflite::micro::GetTensorData<int8_t>(input)[0],
#ifdef USE_TFLM_COMPRESSION
                   tflite::micro::GetTensorData<int8_t>(micro_context, weights, weights_comp_td, weight_scratch_index_),
#else   // USE_TFLM_COMPRESSION
                   tflite::micro::GetTensorData<int8_t>(weights),
#endif  // USE_TFLM_COMPRESSION
                   tflite::micro::GetTensorData<int8_t>(output), ElementCount(*output->dims));
    } break;
...
#ifdef USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int16_t>(micro_context, weights, weights_comp_td, weight_scratch_index_),
#else   // USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int16_t>(weights),
#endif  // USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int16_t>(output), ElementCount(*output->dims));
    } break;
...
#ifdef USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int32_t>(micro_context, weights, weights_comp_td, weight_scratch_index_),
#else   // USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int32_t>(weights),
#endif  // USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int32_t>(output), ElementCount(*output->dims));
    } break;
...
#ifdef USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int64_t>(micro_context, weights, weights_comp_td, weight_scratch_index_),
#else   // USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int64_t>(weights),
#endif  // USE_TFLM_COMPRESSION
          tflite::micro::GetTensorData<int64_t>(output), ElementCount(*output->dims));
    } break;
