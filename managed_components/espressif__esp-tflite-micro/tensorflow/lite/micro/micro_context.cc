...
class MicroContext {
 public:
  ...
  virtual int AllocateDecompressionScratchBuffer(const TfLiteNode *node, int tensor_idx) = 0;

  // Available during Prepare & Eval. Returns nullptr if tensor is not
  // compressed.
  virtual const CompressionTensorData *GetTensorCompressionData(const TfLiteNode *node, int tensor_idx) = 0;

  // Only available during Prepare & Eval. Returns nullptr on failure, otherwise
  // returns a pointer to the buffer.
  virtual void *DecompressTensorToBuffer(const TfLiteEvalTensor &tensor, const CompressionTensorData &compression_data,
                                         void *buffer);
  ...
  virtual TfLiteStatus SetDecompressionMemory(const std::initializer_list<AlternateMemoryRegion> &regions);
  ...
  virtual TfLiteStatus SetAlternateProfiler(MicroProfilerInterface *alt_profiler) { return kTfLiteError; }
  ...
  // The alternate MicroProfilerInterface is currently used by the tensor
  // decompression subsystem.
  virtual MicroProfilerInterface *GetAlternateProfiler() const { return nullptr; }
