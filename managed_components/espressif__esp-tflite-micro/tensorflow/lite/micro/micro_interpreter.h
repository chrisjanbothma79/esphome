...
// compressed.
bool MicroInterpreterContext::IsTensorCompressed(const TfLiteNode *node, int tensor_idx) {
  TFLITE_DCHECK(state_ == InterpreterState::kPrepare || state_ == InterpreterState::kInvoke);

  const SubgraphAllocations *allocations = &graph_.GetAllocations()[graph_.GetCurrentSubgraphIndex()];
...
int MicroInterpreterContext::AllocateDecompressionScratchBuffer(const TfLiteNode *node, int tensor_idx) {
  TFLITE_DCHECK(state_ == InterpreterState::kPrepare);
  ...
  if (AllocateDecompressionMemory(byte_count, MicroArenaBufferAlignment()) != nullptr) {
...
// compressed.
const CompressionTensorData *MicroInterpreterContext::GetTensorCompressionData(const TfLiteNode *node, int tensor_idx) {
  TFLITE_DCHECK(state_ == InterpreterState::kPrepare || state_ == InterpreterState::kInvoke);
...
void *MicroInterpreterContext::DecompressTensorToBuffer(const TfLiteEvalTensor &tensor,
...
TfLiteStatus MicroInterpreterContext::SetDecompressionMemory(
  ...
  decompress_regions_allocations_ = static_cast<size_t *>(AllocatePersistentBuffer(sizeof(size_t) * regions.size()));
...
void *MicroInterpreterContext::AllocateDecompressionMemory(size_t bytes, size_t alignment) {
  TFLITE_DCHECK(state_ == InterpreterState::kPrepare || state_ == InterpreterState::kInvoke);
  if (decompress_regions_ != nullptr) {
    for (size_t i = 0; i < decompress_regions_->size(); i++) {
      ...
      uint8_t *start = static_cast<uint8_t *>(region->address) + decompress_regions_allocations_[i];
...
TfLiteStatus MicroInterpreterContext::SetAlternateProfiler(tflite::MicroProfilerInterface *alt_profiler) {
