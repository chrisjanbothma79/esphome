...
const tflite::micro::compression::Metadata *GetCompressionMetadata(const Model &model) {
  ...
  for (size_t metadata_index = 0; metadata_index < metadata_vector->size(); metadata_index++) {
    ...
    if ((metadata->name()->size() == metadata_string_length) &&
        (std::strncmp(s, kCompressionMetadataString, metadata_string_length) == 0)) {
      ...
      if (vp == nullptr || vp->data() == nullptr) {
        MicroPrintf("Compression: Invalid data for buffer index %u", buffer_index);
        continue;
      }
      ...
      auto compression_metadata = tflite::micro::compression::GetSizePrefixedMetadata(vp);
      flatbuffers::Verifier verifier(vp->data(), vp->size(), flatbuffers::Verifier::Options());
      if (!tflite::micro::compression::VerifyMetadataBuffer(verifier)) {
        ...
        if (compression_metadata->schema_version() > schema.schema_version) {
          MicroPrintf("Compression: schema version mismatch (using %d got %d)", schema.schema_version,
...
TfLiteStatus InitializeCompressionTensorDataFromFlatbuffer(const Model &model, const size_t subgraph_index,
  ...
  if (tensor_index >= tensors->size()) {
    MicroPrintf("Compression: invalid tensor index %u in LutTensor", tensor_index);
    return kTfLiteError;
  }
  const size_t index_bit_width = lut_tensor.index_bitwidth();
  if (index_bit_width > LookupTableData::kMaxBitWidth) {
    MicroPrintf("Compression: invalid bit width %u in LutTensor", index_bit_width);
    return kTfLiteError;
  }
  ...
  if (value_buffer_index >= model.buffers()->size()) {
    MicroPrintf("Compression: invalid value_buffer %u in LutTensor", value_buffer_index);
    return kTfLiteError;
  }
  auto value_buffer = model.buffers()->Get(value_buffer_index)->data();
  if (value_buffer == nullptr || value_buffer->data() == nullptr) {
    MicroPrintf("Compression: invalid value table for value_buffer %u", value_buffer_index);
    return kTfLiteError;
  }
  ctd->data.lut_data->value_table = value_buffer->data();
  auto tensor = model.subgraphs()->Get(subgraph_index)->tensors()->Get(tensor_index);
  ...
  if (tensor->quantization() != nullptr && tensor->quantization()->scale() != nullptr &&
    ...
    const TfLiteIntArray *dims = FlatBufferVectorToTfLiteTypeArray(tensor->shape());
    ...
    if (quantized_axis == 0) {
      ...
      MicroPrintf("Compression: unsupported quantization axis %u", quantized_axis);
      return kTfLiteError;
    }
    ctd->data.lut_data->value_table_channel_stride = (value_buffer->size() / tensor_type_size) / num_channels;
  } else {
    ctd->data.lut_data->is_per_channel_quantized = false;
    ctd->data.lut_data->use_alternate_axis = false;
    ctd->data.lut_data->value_table_channel_stride = value_buffer->size() / tensor_type_size;
  }
...
TfLiteStatus MicroAllocator::AllocateCompressedTensorsList(const Model *model,
                                                           SubgraphAllocations *subgraph_allocations) {
  TFLITE_DCHECK(subgraph_allocations != nullptr);
  ...
  const tflite::micro::compression::Metadata *compression_metadata = internal::GetCompressionMetadata(*model);
  ...
  for (size_t subgraph_index = 0; subgraph_index < compression_metadata->subgraphs()->size(); subgraph_index++) {
    ...
    for (size_t lut_tensors_index = 0; lut_tensors_index < subgraph->lut_tensors()->size(); lut_tensors_index++) {
      auto lut_tensor = subgraph->lut_tensors()->Get(lut_tensors_index);

      CompressionTensorData *ctd =
          reinterpret_cast<CompressionTensorData *>(persistent_buffer_allocator_->AllocatePersistentBuffer(
              sizeof(CompressionTensorData), alignof(CompressionTensorData)));
      if (ctd == nullptr) {
        MicroPrintf("Compressions: failed to allocate memory for "
      ...
      LookupTableData *lut_table = reinterpret_cast<LookupTableData *>(
          persistent_buffer_allocator_->AllocatePersistentBuffer(sizeof(LookupTableData), alignof(LookupTableData)));
      if (lut_table == nullptr) {
        MicroPrintf("Compressions: failed to allocate memory for LookupTableData, "
                    "%d bytes required",
      ...
      TfLiteStatus status =
          internal::InitializeCompressionTensorDataFromFlatbuffer(*model, subgraph_index, *lut_tensor, ctd);
      if (status != kTfLiteOk) {
        MicroPrintf("Compression: failed to initialize data for LutTensor %u", lut_tensors_index);
        return kTfLiteError;
      }

      if (subgraph_allocations[subgraph_index].compressed.tensors == nullptr) {
        size_t alloc_count = model->subgraphs()->Get(subgraph_index)->tensors()->size();
        const CompressionTensorData **tensors =
            reinterpret_cast<const CompressionTensorData **>(persistent_buffer_allocator_->AllocatePersistentBuffer(
                sizeof(CompressionTensorData *) * alloc_count, alignof(CompressionTensorData *)));
        if (tensors == nullptr) {
          MicroPrintf("Compression: failed to allocate memory for compression tensor "
      ...
      const size_t tensor_index = lut_tensor->tensor();
      if (subgraph_allocations[subgraph_index].compressed.tensors[tensor_index] != nullptr) {
        MicroPrintf("Compression: duplicate LutTensor subgraph %u tensor %u", subgraph_index, tensor_index);
        return kTfLiteError;
      } else {
        subgraph_allocations[subgraph_index].compressed.tensors[tensor_index] = ctd;
      }
    }
  }
