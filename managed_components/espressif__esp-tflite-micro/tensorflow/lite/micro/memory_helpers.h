...
void GreedyMemoryPlanner::PrintMemoryPlan() {
  ...
  for (int i = 0; i < buffer_count_; ++i) {
    ...
    MicroPrintf("%c (id=%d): size=%d, offset=%d, first_used=%d last_used=%d", c, i, requirements_[i].size,
                buffer_offsets_[i], requirements_[i].first_time_used, requirements_[i].last_time_used);
  }
  ...
  for (int t = 0; t <= max_time; ++t) {
    ...
    MicroPrintf("%4d: %s (%dk)", t, (const char *) line, (memory_use + 1023) / 1024);
  }
}
