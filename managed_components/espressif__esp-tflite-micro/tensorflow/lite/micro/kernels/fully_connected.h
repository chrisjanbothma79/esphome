...
// code, and only reference the data->is_per_channel in non-HEXAGON code.
#ifdef HEXAGON
    TF_LITE_ENSURE_MSG(context, !is_per_channel,
    ...
    TF_LITE_ENSURE(context, input->type == kTfLiteInt8 && filter->type != kTfLiteInt4);

    TF_LITE_ENSURE_EQ(context, per_channel_quantization_size,
                      filter->dims->data[affine_quantization->quantized_dimension]);

    data->per_channel_output_multiplier = static_cast<int32_t *>(
        context->AllocatePersistentBuffer(context, per_channel_quantization_size * sizeof(int32_t)));
