...
TfLiteStatus ConvPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (micro_context->IsTensorCompressed(node, kConvWeightsTensor) && filter->type == kTfLiteInt4) {
    MicroPrintf("Compression not supported with INT4 tensors");
    return kTfLiteError;
  }
  data->weights_scratch_index = micro_context->AllocateDecompressionScratchBuffer(node, kConvWeightsTensor);
  data->bias_scratch_index = micro_context->AllocateDecompressionScratchBuffer(node, kConvBiasTensor);
