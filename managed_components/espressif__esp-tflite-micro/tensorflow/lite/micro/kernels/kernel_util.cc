...
// tensor->data.
template<typename T>
const T *GetOptionalTensorData(MicroContext *micro_context, const TfLiteEvalTensor *tensor,
  ...
  if (scratch_buffer_handle != -1) {
    ...
    scratch_buffer = micro_context->AllocateDecompressionMemory(bytes_to_allocate, MicroArenaBufferAlignment());
  }
  TFLITE_DCHECK(scratch_buffer != nullptr);
  void *uncompressed_data = micro_context->DecompressTensorToBuffer(*tensor, *compression_data, scratch_buffer);
...
// tensor->data.
template<typename T>
const T *GetTensorData(MicroContext *micro_context, const TfLiteEvalTensor *tensor,
                       const CompressionTensorData *compression_data, int scratch_buffer_handle) {
  TFLITE_DCHECK(tensor != nullptr);
  return GetOptionalTensorData<T>(micro_context, tensor, compression_data, scratch_buffer_handle);
}
