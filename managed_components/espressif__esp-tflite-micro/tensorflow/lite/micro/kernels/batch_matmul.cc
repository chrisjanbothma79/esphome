...
TfLiteStatus ReshapeOutputTensor(TfLiteContext *context, TfLiteNode *node, const RuntimeShape &extended_lhs_shape,
  ...
  TfLiteEvalTensor *output_eval = tflite::micro::GetEvalOutput(context, node, kBatchMatmulOutputTensor);
...
template<typename T> void TransposeRowsColumnsImpl(const TfLiteEvalTensor &tensor_in, TfLiteEvalTensor *tensor_out) {
...
#else
inline TFLMRegistration Register_BATCH_MATMUL_INT8() { return Register_BATCH_MATMUL(); }

inline TFLMRegistration Register_BATCH_MATMUL_INT16() { return Register_BATCH_MATMUL(); }
