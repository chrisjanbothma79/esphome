...
struct PrepareOpContext : OpContext {
  PrepareOpContext(TfLiteContext *context, TfLiteNode *node)
      : OpContext(context, node),
        ...
        lhs(micro_context_->AllocateTempInputTensor(node, kBatchMatmulInputLhsTensor)),
        rhs(micro_context_->AllocateTempInputTensor(node, kBatchMatmulInputRhsTensor)),
        output(micro_context_->AllocateTempOutputTensor(node, kBatchMatmulOutputTensor)) {}
...
struct EvalOpContext : OpContext {
  EvalOpContext(TfLiteContext *context, TfLiteNode *node)
      : OpContext(context, node),
        lhs(tflite::micro::GetEvalInput(context, node, kBatchMatmulInputLhsTensor)),
        rhs(tflite::micro::GetEvalInput(context, node, kBatchMatmulInputRhsTensor)),
        output(tflite::micro::GetEvalOutput(context, node, kBatchMatmulOutputTensor)) {}
...
TfLiteStatus EvalInt8(TfLiteContext *context, const OpDataBatchMatmul &data, const RuntimeShape &lhs_shape,
...
TfLiteStatus EvalInt16(TfLiteContext *context, const OpDataBatchMatmul &data, const RuntimeShape &lhs_shape,
                       const TfLiteEvalTensor &lhs, const RuntimeShape &rhs_shape, const TfLiteEvalTensor &rhs,
                       const RuntimeShape &output_shape, TfLiteEvalTensor *output) {
  TF_LITE_ENSURE(context, data.quantization != nullptr);
  // Reuse params struct from FullyConnected Op.
