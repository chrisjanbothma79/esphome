...
template<typename T>
TfLiteTensor CreateQuantizedBiasTensor(const float *data, T *quantized, TfLiteIntArray *dims, float input_scale,
...
// be quantized.
template<typename T>
TfLiteTensor CreatePerChannelQuantizedBiasTensor(const T *input_data, TfLiteIntArray *dims, float input_scale,
...
TfLiteTensor CreatePerChannelQuantizedBiasTensor(const float *input, T *quantized, TfLiteIntArray *dims,
                                                 float input_scale, const float *weight_scales, float *scales,
...
template<typename T>
TfLiteTensor CreatePerChannelQuantizedTensor(const T *quantized, TfLiteIntArray *dims, TfLiteFloatArray *scales,
                                             TfLiteIntArray *zero_points, TfLiteAffineQuantization *affine_quant,
                                             int quantized_dimension, bool is_variable = false,
                                             TfLiteType type = kTfLiteNoType) {
  affine_quant->scale = scales;
...
template<typename T> struct TestCompressionInfo {
  T *value_table;
...
template<typename T> struct TestCompressionQuantizedInfo : TestCompressionInfo<T> {
...
template<size_t NTENSORS, size_t NINPUTS = 2> class TestCompressedList {
 public:
  template<typename T>
  TfLiteStatus AddInput(const TestCompressionInfo<T> &tci, const TfLiteTensor &tensor, const size_t tensor_index) {
    if (next_input_index_ >= NINPUTS) {
      MicroPrintf("TestCompressedList: too many inputs, max %u", NINPUTS);
      return kTfLiteError;
    }
    inputs_comp_data_[next_input_index_].data.lut_data = &inputs_ltd_[next_input_index_];
    inputs_comp_data_[next_input_index_].scheme = tci.scheme;
    inputs_comp_data_[next_input_index_].data.lut_data->compressed_bit_width = tci.bit_width;
    inputs_comp_data_[next_input_index_].data.lut_data->value_table = tci.value_table;
    inputs_comp_data_[next_input_index_].data.lut_data->value_table_channel_stride = tci.value_table_stride;
    inputs_comp_data_[next_input_index_].data.lut_data->is_per_channel_quantized = IsPerChannelQuantized(tensor);
    inputs_comp_data_[next_input_index_].data.lut_data->use_alternate_axis = UsesAltAxis(tensor);
    return SetCompressionData(tensor_index, inputs_comp_data_[next_input_index_++]);
  }
 ...
 private:
  ...
  TfLiteStatus SetCompressionData(const size_t tensor_index, const CompressionTensorData &cd) {
    ...
    if (ctdp_[tensor_index] != nullptr) {
      MicroPrintf("TestCompressedList: tensor index %d already in use", tensor_index);
      return kTfLiteError;
    }
  ...
  bool IsPerChannelQuantized(const TfLiteTensor &tensor) {
    if (tensor.quantization.type == kTfLiteAffineQuantization && tensor.quantization.params != nullptr) {
      const TfLiteAffineQuantization *qp = static_cast<const TfLiteAffineQuantization *>(tensor.quantization.params);
      if (qp->scale->size > 1) {
        return true;
      }
    }
