...
#define TF_LITE_MICRO_TESTS_END \
  MicroPrintf("%d/%d tests passed", micro_test::tests_passed, (micro_test::tests_failed + micro_test::tests_passed)); \
  ...
  do { \
    ...
    if (isFloatingX && isFloatingY) { \
      ...
      if (delta > std::numeric_limits<decltype(delta)>::epsilon()) { \
        MicroPrintf(#x " == " #y " failed at %s:%d (%f vs %f)", __FILE__, __LINE__, static_cast<double>(vx), \
                    static_cast<double>(vy)); \
        micro_test::did_test_fail = true; \
      } \
    } else if ((vx) != (vy)) { \
      MicroPrintf(#x " == " #y " failed at %s:%d (%d vs %d)", __FILE__, __LINE__, static_cast<int>(vx), \
                  static_cast<int>(vy)); \
      if (isFloatingX || isFloatingY) { \
        MicroPrintf("-----------WARNING-----------"); \
...
#define TF_LITE_MICRO_ARRAY_ELEMENT_EXPECT_NEAR(arr1, idx1, arr2, idx2, epsilon) \
  do { \
    ...
    if (delta > epsilon) { \
      MicroPrintf(#arr1 "[%d] (%f) near " #arr2 "[%d] (%f) failed at %s:%d", static_cast<int>(idx1), \
                  static_cast<float>((arr1)[(idx1)]), static_cast<int>(idx2), static_cast<float>((arr2)[(idx2)]), \
                  __FILE__, __LINE__); \
      micro_test::did_test_fail = true; \
    } \
  } while (false)
...
#define TF_LITE_MICRO_EXPECT_NEAR(x, y, epsilon) \
  do { \
    ...
    if (vx != vy && delta > epsilon) { \
      MicroPrintf(#x " (%f) near " #y " (%f) failed at %s:%d", static_cast<double>(vx), static_cast<double>(vy), \
                  __FILE__, __LINE__); \
      micro_test::did_test_fail = true; \
    } \
  } while (false)
  ...
  do { \
    for (int i = 0; string1[i] != '\0' && string2[i] != '\0'; i++) { \
      if (string1[i] != string2[i]) { \
        MicroPrintf("FAIL: %s did not match %s", string1, string2, __FILE__, __LINE__); \
        micro_test::did_test_fail = true; \
        break; \
      } \
    } \
